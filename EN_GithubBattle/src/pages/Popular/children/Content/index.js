...
class Content extends Component {
    ...
    render() {
      ...
      const lightStyle = { background: 'rgba(0, 0, 0, 0.08)', borderRadius: '3px', color: '#000' };
      const darkStyle = { background: 'rgb(36, 40, 42)', borderRadius: '3px', color: '#fff' };
      const liStyle = { margin: '10px', display: 'flex', alignItems: 'center' };
      const blockStyle = bgStyle === 'light' ? lightStyle : darkStyle;

      return (
        <div style={{
          marginTop: '30px', width: '100%', display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap',
        }}
        >
          {
                    currentContent.length ? (
                      currentContent.map((item, key) => (
                        <div
                          key={key}
                          style={{
                            margin: '10px 5px', padding: '20px', width: '250px', textAlign: 'center', ...blockStyle,
                          }}
                        >
                          <h4 style={{ margin: '20px', fontSize: '35px', fontWeight: '300' }}>#{key + 1}</h4>
                          <img
                            style={{
                              marginBottom: '8px', width: '150px', height: '150px', borderRadius: '3px',
                            }}
                            src={item.owner.avatar_url || 'https://img.zcool.cn/community/01136858d4d266a801219c7766cc7f.gif'}
                            alt=""
                          />
                          <h2><a href={item.owner.html_url} style={{ color: 'rgb(187, 46, 31)', fontWeight: 'bold', cursor: 'pointer' }}>{item.owner.login}</a></h2>
