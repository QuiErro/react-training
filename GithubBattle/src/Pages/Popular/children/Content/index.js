...
export default class Content extends React.Component{
    ...
    render(){
        ...
        let lightStyle = {background: 'rgba(0, 0, 0, 0.08)', borderRadius: '3px', color: '#000'};
        let darkStyle = {background: 'rgb(36, 40, 42)', borderRadius: '3px', color: '#fff'};
        let liStyle = {margin: '10px', display: 'flex', alignItems: 'center'};
        let blockStyle = bgStyle === 'light' ? lightStyle : darkStyle;
            ...
            <div style={{marginTop: '30px', width: '100%', display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap'}}>
                {
                    currentContent.length ? (
                        currentContent.map((item, key)=>{
                            return (
                                <div key={key} style={{margin: '10px 5px', padding: '20px', width: '250px', textAlign: 'center', ...blockStyle}}>
                                    <h4 style={{margin: '20px', fontSize: '35px', fontWeight: '300'}}>#{key + 1}</h4>
                                    <img style={{marginBottom: '8px', width: '150px', height: '150px', borderRadius: '3px'}} src={item.owner.avatar_url || "https://img.zcool.cn/community/01136858d4d266a801219c7766cc7f.gif"} alt="" />
                                    ...
                                    <ul style={{margin: '20px 0', fontSize: '20px'}}>
                                        <li style={liStyle}>
                                            <div style={{display: 'flex', alignItems: 'center'}}>
                                                ...
                                                <a href={item.owner.html_url} style={{color: 'inherit', fontWeight: '600', 'cursor': 'pointer'}}>{item.owner.login}</a>
                                            </div>
                                        </li>
                                        <li style={liStyle}>
                                            ...
                                            <span>{this._dealWithNumber(item.stargazers_count)}&nbsp;stars</span>
                                        </li>
                                        <li style={liStyle}>
                                            ...
                                            <span>{this._dealWithNumber(item.forks_count)}&nbsp;forks</span>
                                        </li>
                                        <li style={liStyle}>
                                            ...
                                            <span>{this._dealWithNumber(item.open_issues)}&nbsp;open issues</span>
                                        </li>
                                    </ul>
                                </div>
                            )
